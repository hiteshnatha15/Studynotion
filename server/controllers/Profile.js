...
exports.deleteAccount = async (req, res) => {
  try {
    ...
    const uid = new mongoose.Types.ObjectId(user.additionalDetails);
    console.log(uid);
    const response = await Profile.findByIdAndDelete({ _id: uid });
    ...
    courses.forEach(async (course) => {
      const index = course.studentsEnrolled.indexOf(id);
      if (index > -1) {
